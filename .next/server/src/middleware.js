...
;// CONCATENATED MODULE: ./src/lib/constant.ts
const constant_CONST = Object.freeze({
    BASE_URL:  false ? 0 : "https://api.swayauth.com/v1",
    CLIENT_BASE_URL:  false ? 0 : "https://swayauth.com",
    ...
    AUTH: {
        ...
        GOOGLE:  false ? 0 : "https://api.swayauth.com/v1/auth/google?client_id=",
        FACEBOOK:  false ? 0 : "https://api.swayauth.com/v1/auth/facebook?client_id="
    },
    ACCOUNT: {
        PHOTO: "/account/photo",
        GET_PROFILE: "/account",
...
;// CONCATENATED MODULE: ./src/lib/token.ts
...
const isGoodToken = (token)=>{
    try {
        const tk = token ?? js_cookie_api.get(constant_CONST.ACCESS_TOKEN) ?? "";
        return Date.now() < jwtDecode(tk)?.exp * 1000;
    } catch (error) {
        return false;
    }
};
...
const auth_redirects = [
    constant_CONST.LOCATION.LOGIN,
    constant_CONST.LOCATION.VERIFY,
    constant_CONST.LOCATION.FORGOT_PASSWORD,
    constant_CONST.LOCATION.REGISTER
];
const auth_pages = [
    constant_CONST.LOCATION.CLIENT_AREA
];
async function middleware(req) {
    const token = req.cookies.get(constant_CONST.ACCESS_TOKEN)?.value ?? "";
    const status = isGoodToken(token);
    const pathname = req.nextUrl?.pathname ?? "";
    const url = req.nextUrl?.clone();
    if (auth_redirects.some((link)=>pathname.includes(link)) && status) {
        url.pathname = constant_CONST.LOCATION.CLIENT_AREA;
        return NextResponse.redirect(url);
    }
    if (auth_pages.some((link)=>pathname.includes(link))) {
        if (!status) {
            url.pathname = constant_CONST.LOCATION.LOGIN;
            return NextResponse.redirect(url);
        }
    }
}

;// CONCATENATED MODULE: ./node_modules/next/dist/build/webpack/loaders/next-middleware-loader.js?absolutePagePath=private-next-root-dir%2Fsrc%2Fmiddleware.ts&page=%2Fsrc%2Fmiddleware&rootDir=%2FUsers%2Fjohn%2Fwork%2Fswayauth%2Fclient&matchers=&preferredRegion=&middlewareConfig=e30%3D!
