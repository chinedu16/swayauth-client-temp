          ...
          return <div key={fidx}>
            ...
            </div>
            {
              folder.item.map((api, aidx) => {
                return <div key={aidx} className='flex flex-wrap mb-14'>
                  <div className='w-full lg:w-3/6 lg:pr-6 mb-10 lg:mb-0'>
                    <h3 id={idMaker(api.request.method + api.request.url.raw)} className='text-lg mb-4'><span className={`${api.request.method} mr-3`}>{api.request.method}</span>{api.name}</h3>
