...
const VerifySmtp = ({ isOpen, message, toggle, handle2faVerify, onChange, loading, length = 6 }: { handle2faVerify?: () => void, message?: string, loading?: boolean, isOpen: boolean, length?: number, onChange?: (v: string) => void, toggle: () => void }) => {
    ...
    <div className="mx-auto transition w-full items-center justify-center flex" >
      <div className="bg-white rounded-md w-full">
        <div className="flex items-center px-4 py-2 w-full border-b">
          <h2 className='text-xl pl-3 font-bold'>Verify SMTP Service</h2>
        </div>
        <div className="px-7 pt-7 pb-6">
          <div className="text-center mt-4">
            <p>Enter the 6-digit authentication code sent to your mail box:</p>
          </div>
