...
const B2B = () => {
  ...
  const defaultPage = (searchParams.get("default") || "login") as
    ...
    | "register"
    | "forgot";
  const templateParam = (searchParams.get("template") || "minimal") as
    | "classic"
    | "modern"
    | "minimal";

  return (
    <HostedAuthB2B appId={appId} redirectUrl={redirectUrl} defaultPage={defaultPage} templateParam={templateParam} />
  );
};
...
const HostedAuthB2B = ({
  ...
  const title = config?.name ? `${config.name} (B2B)` : "Swayauth (B2B)";
